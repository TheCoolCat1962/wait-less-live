...
// EXCLUDED_TYPES_SET removed to allow all legitimate business types.
...
const CATEGORY_MAP: Record<string, { label: string; emoji: string }> = {
  ...
  karaoke_bar: { label: "Karaoke", emoji: "🎤" },
  nightclub: { label: "Nightclub", emoji: "🌃" },
  casino: { label: "Casino", emoji: "🎰" },

  hair_salon: { label: "Hair Salon", emoji: "💇" },
  barber_shop: { label: "Barber", emoji: "✂️" },
  nail_salon: { label: "Nails", emoji: "💅" },
  tattoo_shop: { label: "Tattoo", emoji: "🖍️" },
  spa: { label: "Spa", emoji: "🧖" },

  donut_shop: { label: "Donuts", emoji: "🍩" },
  seafood_restaurant: { label: "Seafood", emoji: "🦞" },
  ramen_restaurant: { label: "Ramen", emoji: "🍜" },
  sushi_restaurant: { label: "Sushi", emoji: "🍣" },
  thai_restaurant: { label: "Thai", emoji: "🍲" },
  mexican_restaurant: { label: "Mexican", emoji: "🌮" },
  chinese_restaurant: { label: "Chinese", emoji: "🥢" },
  indian_restaurant: { label: "Indian", emoji: "🍛" },
  italian_restaurant: { label: "Italian", emoji: "🍝" },
  korean_restaurant: { label: "Korean", emoji: "🥘" },
  japanese_restaurant: { label: "Japanese", emoji: "🍱" },
  vietnamese_restaurant: { label: "Vietnamese", emoji: "🍲" },
  french_restaurant: { label: "French", emoji: "🥖" },
  creole_restaurant: { label: "Creole", emoji: "🍲" },
  cajun_restaurant: { label: "Cajun", emoji: "🌶️" },
  bubble_tea_shop: { label: "Bubble Tea", emoji: "🧋" },
  juice_bar: { label: "Juice Bar", emoji: "🥤" },
  smoothie_shop: { label: "Smoothies", emoji: "🥤" },
  liquor_store: { label: "Liquor", emoji: "🍷" },
  wine_bar: { label: "Wine Bar", emoji: "🍇" },
  beer_bar: { label: "Beer Bar", emoji: "🍺" },
  cocktail_bar: { label: "Cocktails", emoji: "🍹" },

  toy_store: { label: "Toys", emoji: "🧸" },
  pet_store: { label: "Pet Store", emoji: "🐾" },
  flower_shop: { label: "Flowers", emoji: "🌹" },
  jewelry_store: { label: "Jewelry", emoji: "💎" },
  watch_store: { label: "Watches", emoji: "⌚" },
  sporting_goods_store: { label: "Sports", emoji: "⚽" },
  outdoor_gear_store: { label: "Outdoor", emoji: "⛺" },
  music_store: { label: "Music", emoji: "🎸" },
  antique_store: { label: "Antiques", emoji: "🏺" },
  thrift_store: { label: "Thrift", emoji: "👕" },

  laundromat: { label: "Laundromat", emoji: "🧺" },
  car_wash: { label: "Car Wash", emoji: "🚗" },
  car_repair: { label: "Car Repair", emoji: "🔧" },
  auto_parts_store: { label: "Auto Parts", emoji: "🛠️" },
  dry_cleaning: { label: "Dry Cleaning", emoji: "👔" },
  shoe_repair: { label: "Shoe Repair", emoji: "👞" },
  printing_shop: { label: "Printing", emoji: "🖨️" },

  dentist: { label: "Dentist", emoji: "🦷" },
  optometrist: { label: "Optometrist", emoji: "👓" },
  veterinarian: { label: "Veterinarian", emoji: "🐾" },
  yoga_studio: { label: "Yoga", emoji: "🧘" },

  library: { label: "Library", emoji: "📚" },
  museum: { label: "Museum", emoji: "🖼️" },
  art_gallery: { label: "Gallery", emoji: "🎨" },
  theater: { label: "Theater", emoji: "🎭" },
  concert_hall: { label: "Concert Hall", emoji: "🎵" },
};
...
export const fetchNearbyBusinesses = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const places = (json.places ?? []).filter(
      (p) => p.id && p.location && p.displayName?.text,
    );
    ...
    const results = stored.map((b) => {
      const agg = aggregateReports(byBiz.get(b.id) ?? []);
      const score = scoreSearchResult(b.name, data.query, data.lat, data.lng, b.lat, b.lng, agg.count);
      return {
        ...b,
        currentMinutes: agg.current,
        updatedMinutesAgo: agg.latest ? Math.max(0, (Date.now() - new Date(agg.latest).getTime()) / 60_000) : null,
...
// ---------------------------------------------------------------------------
// Search ranking helper — prioritizes exact matches, nearby businesses, and
// businesses with recent wait-time reports.
// ---------------------------------------------------------------------------
function scoreSearchResult(
  ...
  // Query is a substring of name: +25
  else if (nameLower.includes(queryLower)) score += 25;

  // Distance bonus: closer = higher score (up to +30)
  const lat1 = (userLat * Math.PI) / 180;
  const lat2 = (placeLat * Math.PI) / 180;
  const deltaLat = ((placeLat - userLat) * Math.PI) / 180;
  const deltaLng = ((placeLng - userLng) * Math.PI) / 180;
  const a =
  ...
  // Report count bonus: +10 per report (capped at +50)
  score += Math.min(50, reportCount * 10);
...
export const searchBusinessesByText = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Sort by custom scoring: exact matches, nearby businesses, and popular businesses.
    // We'll apply the scoring after we have report counts.
