    ...
    <AppShell>
      ...
      <main className="px-5 py-4">
        {reputationQuery.isLoading ? (
          ...
          <div className="mx-5 mt-4 rounded-xl border border-danger/30 bg-danger/10 p-4 text-center">
            <p className="text-sm font-medium text-danger">Failed to load reputation.</p>
            <button onClick={() => reputationQuery.refetch()} className="mt-2 text-sm font-bold text-brand">Retry</button>
          </div>
        ) : !data ? (
          <div className="text-center py-16 text-sm text-muted-foreground">No data found.</div>
        ) : activeTab === "stats" ? (
